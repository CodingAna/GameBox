...
export class CustomDraw {
  ...
  rectAnimation2 = (id, duration, mode, delta, p1, p2, p1t, p2t, fill) => {
    if (this._animationProgress[id] === undefined) this._animationProgress[id] = {value: 0, increase: true};

    // Modes: 0 = Endless
    //        1 = Oneshot + Reverse
    //        2 = Oneshot

    let percentPerSecond = 1 / duration;
    let secondsPerFrame = delta;
    let percentPerFrame = percentPerSecond * secondsPerFrame;

    this._animationProgress[id].value += percentPerFrame * (this._animationProgress[id].increase ? 1 : -1);

    if (this._animationProgress[id].value > 1) {
      this._animationProgress[id].value = 1;

      if (mode === 2) {
        this.rect(p1t, p2t, fill);
        return;
      }

      if (mode === 1 && this._animationProgress[id].increase) {
        this._animationProgress[id].increase = false;
      }
    ...
    if (this._animationProgress[id].value < 0) {
      this._animationProgress[id].value = 0;

      if (mode === 1) {
        this.rect(p1, p2, fill);
        return;
      }

      if (mode === 0) {
        this._animationProgress[id].increase = true;
      }
    }

    let ip1 = p1.interpolate(p1t, this._animationProgress[id].value);
    let ip2 = p2.interpolate(p2t, this._animationProgress[id].value);

    this.rect(ip1, ip2, fill);
  }

  rectAnimation = (progress, p1, p2, p3, p4, fill) => {
    let cps = p1.interpolate(p3, progress);
    let cpt = p2.interpolate(p4, progress);
    this.rect(cps, cpt, fill);
  }
